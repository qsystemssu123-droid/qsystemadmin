...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-sky-50 text-slate-800 font-sans overflow-hidden">
      {/* Light Blue Tint Sidebar */}
      <aside
        className={`${
          sidebarOpen ? 'w-64' : 'w-20'
        } bg-white border-r border-sky-100 transition-all duration-300 flex flex-col z-20 shadow-xs`}
      >
        <div className="h-16 flex items-center px-6 bg-white text-sky-700 font-bold text-lg tracking-wide border-b border-sky-100">
          {sidebarOpen ? 'QueueMatrix Admin' : 'QM Admin'}
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium cursor-pointer transition-all duration-200 ease-in-out transform hover:translate-x-1 ${
                  activeTab === item.id
        ...
        <div className="p-3 border-t border-sky-100">
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-rose-600 hover:bg-rose-50 cursor-pointer transition-all duration-200 ease-in-out transform hover:translate-x-1"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Header */}
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-sky-100 flex items-center justify-between px-6 z-10 shadow-2xs">
          <button
